  ...
  return (
    <section className="news-page">
      ...
      <div className="panel detail-card">
        ...
        {stocks.map((stock) => <button className="catalyst-row" onClick={() => onSelectStock(stock.symbol)} key={stock.symbol}><strong>{stock.symbol}</strong><span>{stock.catalysts[0]}</span><b className={stock.change >= 0 ? 'up' : 'down'}>{stock.change >= 0 ? '+' : ''}{stock.change}%</b></button>)}
      </div>
    </section>
  )
}
